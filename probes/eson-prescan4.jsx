// Per-pass profiler v4: current code, incl. parseJson-inline vs facade delta.
#target illustrator
...
function parseJsonInline(text) {
  var raw = String(text);
  if (!preScanFull(raw)) return false;
  rx_dangerous.lastIndex = 0;
  var s = rx_dangerous.test(raw) ? raw.replace(rx_dangerous, function (a) { return '\\u' + ('0000' + a.charCodeAt(0).toString(16)).slice(-4); }) : raw;
  return eval('(' + s + ')');
}
...
rows.push({ pass: 'parseJson-inline', medianUs: timeLane(function () { parseJsonInline(t); }, 3, 12) });
rows.push({ pass: 'eson.parse-facade', medianUs: timeLane(function () { ESON.parse(t); }, 3, 12) });
...
var rf = new File($.getenv('TEMP') + '/eson-prescan4.json');
rf.encoding = 'UTF-8';
rf.open('w');
